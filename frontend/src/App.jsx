...
export default function App() {
  ...
  // Bumped whenever KYC is (re)submitted in this session, so the fund recommendation
  // (gated on KYC status) refetches instead of showing a status from before the submit.
  const [kycVersion, setKycVersion] = useState(0)
  ...
  function loadInfo() {
    setInfoError(null)
    api.info().then(setInfo).catch((e) => setInfoError(e.message))
  }

  return (
    <main className="wrap">
      <h1>Robo-advisor (learning build)</h1>
      ...
      {user && (
        <>
          ...
          {profileError && <p className="err">Could not load your profile: {profileError}</p>}
          {!profileError && assessment === undefined && <p>Loading your profile...</p>}
          {assessment === null && <RiskQuestionnaire onSaved={handleSaved} />}
          {assessment && !retaking && (
            <RiskResult assessment={assessment} previousCount={previousCount} onRetake={() => setRetaking(true)} />
          )}
          {assessment && retaking && <RiskQuestionnaire onSaved={handleSaved} onCancel={() => setRetaking(false)} />}
      ...
      <details onToggle={(e) => e.currentTarget.open && !info && loadInfo()}>
        ...
        {infoError && <p className="err">Could not reach the API: {infoError}</p>}
